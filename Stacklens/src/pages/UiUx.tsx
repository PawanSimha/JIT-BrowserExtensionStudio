...
import { Palette, Type, Copy, Check, Layers, Sparkles, Ruler, Square, Moon, Wind, Activity, Grid3x3, Box, Droplets, PaintBucket } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {rgb && <span className="text-2xs text-text-muted font-mono">rgb({rgb})</span>}
      </div>
      <span className="text-2xs text-text-secondary truncate max-w-[100px] text-right">{usage}</span>
    </button>
  );
}
    ...
    <div className="flex items-center gap-3 px-2 py-2 rounded-lg border border-surface-border">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="badge text-[9px] text-text-muted bg-surface-hover">{usage}</span>
        </div>
        <p className="text-sm text-text-primary mt-0.5 truncate" style={{ fontFamily: family }}>
...
function TypographyCard({ level, family, size, weight, count }: { level: string; family: string; size: string; weight: string; count: number }) {
  ...
  return (
    <div className="flex items-center gap-3 px-2 py-1.5 rounded-lg border border-surface-border">
      <div className="w-9 h-9 rounded-lg bg-surface-hover flex items-center justify-center shrink-0 text-[9px] font-bold font-heading text-text-muted uppercase">
        {level}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 text-xs">
          <span className="font-medium text-text-primary">{levelLabels[level]}</span>
          {weight && <span className="text-2xs text-text-muted font-code">w{weight}</span>}
        </div>
        <span className="text-2xs text-text-secondary">{size} · {family || 'system'}</span>
      </div>
      <span className="text-2xs text-text-muted font-mono">{count}x</span>
    </div>
  );
}
      ...
      {values.map((v) => {
        ...
        return (
          <div key={v.value} className="flex flex-col items-center gap-1" style={{ width: 40 }}>
            <div
              className="w-full rounded-t bg-brand-primary/30 border border-brand-primary/50"
              style={{ height }}
...
function TokenRow({ name, value, category }: { name: string; value: string; category: string }) {
  return (
    <div className="flex items-center gap-2 px-2 py-1 rounded hover:bg-surface-hover transition-colors">
      <span className="text-2xs font-code text-brand-primary shrink-0 min-w-[100px] truncate" title={name}>
        {name}
      </span>
...
export default function UiUx() {
  ...
  if (!currentResult) {
    return (
      <div className="page-container flex flex-col items-center justify-center text-center gap-3">
        ...
        <h2 className="text-base font-semibold font-heading text-text-primary">UI/UX Design</h2>
  ...
  if (!hasColors && !hasFonts && !hasExtra) {
    return (
      <div className="page-container flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <Palette size={14} className="text-brand-primary" strokeWidth={1.5} />
          <h2 className="text-sm font-semibold font-heading text-text-primary">{currentResult.hostname}</h2>
        </div>
        <div className="flex-1 flex items-center justify-center">
          <p className="text-xs text-text-secondary">No design tokens detected on this page.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-container flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3 px-3 py-2 rounded-lg border border-surface-border bg-surface-card">
        <Layers size={16} className="text-brand-accent shrink-0" strokeWidth={1.5} />
      ...
      <div className="flex-1 overflow-y-auto scrollable space-y-3">
        ...
        {accentColors.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-2 flex items-center gap-1">
              <Sparkles size={11} strokeWidth={1.5} /> Color Palette
            </h3>
        ...
        {textColors.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5">
        ...
        {bgColors.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5">
              Background Colors ({bgColors.length})
            </h3>
            <div className="flex flex-col gap-0.5">
        ...
        {borderColors.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5">
              Border Colors ({borderColors.length})
            </h3>
            <div className="flex flex-col gap-0.5">
        ...
        {otherColors.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5">
              Other Colors ({otherColors.length})
            </h3>
            <div className="flex flex-col gap-0.5">
        ...
        {contrastPairs.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Activity size={11} strokeWidth={1.5} /> Contrast Analysis
            </h3>
            <div className="flex flex-col gap-1">
              {contrastPairs.map((p, i) => {
                const level = getContrastLevel(p.ratio);
                const color = getContrastColor(p.ratio);
                return (
                  <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg border border-surface-border">
                    ...
                    <span className="text-2xs text-text-muted">{p.fg} on {p.bg}</span>
                    <span className={`text-2xs font-medium ml-auto ${color}`}>
        ...
        {design!.typography && design!.typography.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Type size={11} strokeWidth={1.5} /> Typography Scale
            </h3>
            <div className="flex flex-col gap-1">
        ...
        {design!.spacing && design!.spacing.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Ruler size={11} strokeWidth={1.5} /> Spacing Scale
            </h3>
        ...
        {design!.borderRadius && design!.borderRadius.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Square size={11} strokeWidth={1.5} /> Border Radius
            </h3>
            <div className="flex flex-wrap gap-1.5">
              {design!.borderRadius.map((r, i) => (
                <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg border border-surface-border">
                  <div className="w-5 h-5 border border-brand-primary/40 shrink-0" style={{ borderRadius: r.value }} />
                  <div>
                    <span className="text-xs text-text-primary font-medium">{r.value}</span>
                    <span className="text-2xs text-text-muted ml-1">({r.label})</span>
                  </div>
                  <span className="text-2xs text-text-muted ml-auto">{r.count}x</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {design!.boxShadows && design!.boxShadows.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Box size={11} strokeWidth={1.5} /> Box Shadows
            </h3>
            <div className="flex flex-col gap-1">
              {design!.boxShadows.map((s, i) => (
                <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg border border-surface-border">
                  <div className="w-5 h-5 rounded bg-surface-card border border-surface-border shrink-0" style={{ boxShadow: s.css }} />
                  <span className="text-xs text-text-primary font-medium">{s.label}</span>
                  <span className="text-2xs text-text-muted ml-auto">{s.count}x</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {design!.gradients && design!.gradients.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Droplets size={11} strokeWidth={1.5} /> Gradients
            </h3>
            <div className="flex flex-col gap-1">
              {design!.gradients.map((g, i) => (
                <div key={i} className="px-2 py-2 rounded-lg border border-surface-border">
                  ...
                  <div className="flex gap-1 mt-1">
                    ...
                    <span className="text-2xs text-text-muted ml-auto">{g.type}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {design!.darkMode && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Moon size={11} strokeWidth={1.5} /> Dark Mode
            </h3>
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg border border-surface-border">
              <div className="w-8 h-8 rounded-lg bg-surface-card flex items-center justify-center">
                <Moon size={14} className="text-indigo-400" strokeWidth={1.5} />
              </div>
              <div>
                <p className="text-xs font-medium text-text-primary">Supported</p>
                <p className="text-2xs text-text-muted">
                  Method: {design!.darkMode.method === 'class' ? 'Class toggle' : design!.darkMode.method === 'attribute' ? 'Attribute toggle' : 'Media query'}
                  {design!.darkMode.toggleClass && <span className="ml-1 font-code">({design!.darkMode.toggleClass})</span>}
                </p>
              </div>
              <span className="badge bg-green-500/10 text-green-400 text-[9px] ml-auto">Detected</span>
            </div>
          </div>
        )}
        ...
        {design!.layout && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Grid3x3 size={11} strokeWidth={1.5} /> Layout System
            </h3>
            <div className="px-3 py-2 rounded-lg border border-surface-border">
              <div className="flex gap-1 h-2 rounded-full overflow-hidden mb-2">
                {design!.layout.flexboxPct > 0 && (
                  <div style={{ width: design!.layout.flexboxPct + '%' }} className="bg-blue-500/60 first:rounded-l-full" title={`Flexbox ${design!.layout.flexboxPct}%`} />
                )}
                {design!.layout.gridPct > 0 && (
                  <div style={{ width: design!.layout.gridPct + '%' }} className="bg-purple-500/60 last:rounded-r-full" title={`Grid ${design!.layout.gridPct}%`} />
                )}
              </div>
              <div className="flex gap-3 text-2xs text-text-secondary">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500/60" /> Flexbox {design!.layout.flexboxPct}%</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-500/60" /> Grid {design!.layout.gridPct}%</span>
        ...
        {design!.animation && (design!.animation.durations.length > 0 || design!.animation.easings.length > 0) && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Wind size={11} strokeWidth={1.5} /> Animation Profile
            </h3>
            <div className="grid grid-cols-2 gap-1">
              {design!.animation.durations.length > 0 && (
                <div className="px-2 py-1.5 rounded-lg border border-surface-border">
                  <p className="text-2xs text-text-muted mb-1 font-medium">Durations</p>
                  {design!.animation.durations.map((d, i) => (
                    <div key={i} className="flex justify-between text-2xs text-text-primary">
                      ...
                      <span className="text-text-muted">{d.count}x</span>
                    </div>
                  ))}
                </div>
              )}
              {design!.animation.easings.length > 0 && (
                <div className="px-2 py-1.5 rounded-lg border border-surface-border">
                  <p className="text-2xs text-text-muted mb-1 font-medium">Easings</p>
                  {design!.animation.easings.map((e, i) => (
                    <div key={i} className="flex justify-between text-2xs text-text-primary">
                      <span className="font-code truncate">{e.value}</span>
                      <span className="text-text-muted ml-1">{e.count}x</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        {tokens.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <PaintBucket size={11} strokeWidth={1.5} /> Design Tokens
            </h3>
            <div className="rounded-lg border border-surface-border overflow-hidden">
              {tokenCategories.map((cat) => {
                ...
                return (
                  <div key={cat} className="border-b border-surface-border last:border-b-0">
                    <div className="px-2.5 py-1 bg-surface-card">
                      <span className="text-[9px] font-semibold text-text-secondary uppercase tracking-wider">{cat}</span>
                      <span className="text-2xs text-text-muted ml-1">({catTokens.length})</span>
                    </div>
              ...
              {tokens.length > 16 && (
                <div className="px-2.5 py-1 text-center text-2xs text-text-muted">
        ...
        {allFonts.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold font-heading text-text-secondary uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <Type size={11} strokeWidth={1.5} /> Fonts ({allFonts.length})
            </h3>
            <div className="flex flex-col gap-1">
